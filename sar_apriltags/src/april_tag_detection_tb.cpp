...
const char* windowName = "apriltags_tb_demo";
...
public:
  ...
  void print_detection(AprilTags::TagDetection& detection) const {
    ...
    //Transform stuff
    geometry_msgs::PointStamped camera_p;
    camera_p.header.frame_id = "laser";
    camera_p.point.x = translation(0);
    camera_p.point.y = translation(1);
    camera_p.point.z = translation(2);
    geometry_msgs::PointStamped loc_stamped;
    tf_listener->transformPoint("/map", camera_p, loc_stamped);
    
    //Message to publish the APril tag ID's collected
    wasp_custom_msgs::object_loc location;
    location.ID = detection.id;
    location.point = loc_stamped.point;
    object_location_pub.publish(location);

    cout << "  distance=" << translation.norm()
         << "m, x=" << loc_stamped.point.x
         << ", y=" << loc_stamped.point.y
         << ", z=" << loc_stamped.point.z
         << ", yaw=" << yaw
         << ", pitch=" << pitch
         << ", roll=" << roll
         << endl;
...
int main(int argc, char* argv[]) {
  ros::init(argc, argv, "Tb_Tag_Detector");
  ros::NodeHandle nh;
  ...
  object_location_pub = nh.advertise<wasp_custom_msgs::object_loc>("object_location_tb", 1);
